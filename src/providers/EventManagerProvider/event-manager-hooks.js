...
export const useLogMessageToL2Listener = () => {
  ...
  return useCallback(callback => addListener(EventName.L1.LOG_MESSAGE_TO_L2, callback), []);
};

export const useLogDepositListener = () => {
  const {addListener} = useContext(EventManagerContext);

  return useCallback(callback => addListener(EventName.L1.LOG_DEPOSIT, callback), []);
};

export const useLogWithdrawalListener = () => {
  const {addListener} = useContext(EventManagerContext);

  return useCallback(callback => addListener(EventName.L1.LOG_WITHDRAWAL, callback), []);
};
